...
import {Flatten} from 'canvas-dither.js';
...
class EscPosEncoder {
  ...
  _flush() {
    if (this._embedded) {
      let indent = this._options.width - this._cursor;
        //freshka 20220708
        //console.log('_flush() indent = '+indent + ' ::: this._options.width = '+this._options.width+ ' ::: this._cursor = '+this._cursor);
  ...
  text(value, wrap) {
    ...
    for (let l = 0; l < lines.length; l++) {
      ...
      /*const krLines = lines[l].replace(/[^\uAC00-\uD7AF\u1100-\u11FF\u3130-\u318F]/gi,"");
      if (this._state.hanzi) {
          this._cursor += (krLines.length * this._state.width);
      }
      else {
      ...
      for (let c = 0; c < columns.length; c++) {
        ...
        if (typeof data[r][c] === 'function') {
          ...
          const encoded = columnEncoder.encode();
            ...
            //freshka 20220708
            //let columnWidth = columns[c].width;
            //console.log('table(columns, data) encoded.byteLength = '+encoded.byteLength + ' :::  + columns[c].width = '+ columnWidth);
